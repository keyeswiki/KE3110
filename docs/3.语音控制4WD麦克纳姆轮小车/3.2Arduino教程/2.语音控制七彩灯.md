...
mecanumCar mecanumCar(3, 2);  //sda-->D3,scl-->D2
```
引入了 `SoftwareSerial` 库，因为它允许在任意数字引脚上模拟串口通信。`SoftwareSerial mySerial(RX_PIN, TX_PIN)` 表示 A5 作为 RX（接收），A4 作为 TX（发送）。这解决了 Arduino 只有一个硬件串口且被 USB 占用的问题。
...
```
如果语音模块接收到 “请开灯” 等命令词(命令码为1)时，七彩灯闪烁；如果语音模块接收到 “请关灯” 等命令词(命令码为2)时，七彩灯熄灭。
